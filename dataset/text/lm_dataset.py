...
class CharLevelLanguageModelTestState:
    ...
    def step(self, net_out: torch.Tensor, data: Dict[str, torch.Tensor]):
        with torch.no_grad():
            ...
            self.n_ok += (net_out.argmax(-1) == target).float().sum().cpu().item()
...
class CharLanguageDataset(torch.utils.data.Dataset):
    VERISON = 1
    ...
    def get_version(self) -> Optional[int]:
        verfile = os.path.join(self.cache_dir, "version")
    ...
    def load(self):
        ...
        if self.get_version() != self.VERISON:
            ...
            for f in self.get_all_files():
                split_fn = os.path.join(self.cache_dir, f)

                with self.open_file(split_fn) as f:
                    for l in f:
                        voc.update(self.get_tokens(l))

            self.__class__.vocabulary = self.create_vocab(voc)
            ...
            torch.save(voc, voc_cache)
            self.write_version()
        else:
            voc = torch.load(voc_cache)
            self.__class__.vocabulary = self.create_vocab(voc)
            self.update_data_type()
