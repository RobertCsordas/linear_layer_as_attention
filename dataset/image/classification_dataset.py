...
class ImageClassificationDataset(torch.utils.data.Dataset):
    ...
    def get_download_completed_path(self, cache: str) -> pathlib.Path:
        return pathlib.Path(os.path.join(cache, "download_completed"))

    def check_if_downloaded(self, cache: str) -> bool:
        return self.get_download_completed_path(cache).exists()
    ...
    def __init__(self, set: str, cache: str = "./cache", valid_split_size: float = 0.2,
        ...
        with LockFile(os.path.join(cache, "download_lock")):
            if not self.check_if_downloaded(cache):
                self.download(cache)
                self.get_download_completed_path(cache).touch()
    ...
    def start_test(self) -> ImageSetTestState:
        return ImageSetTestState(self)
    ...
    def measure_mean_std(self, cache):
        ...
        self.mean_tensor = np.expand_dims(np.expand_dims(mean, -1), -1).astype(np.float32)
        self.std_tensor = np.expand_dims(np.expand_dims(std, -1), -1).astype(np.float32)

    def unnormalize(self, img: torch.Tensor) -> torch.Tensor:
        return img * self.std_tensor + self.mean_tensor
